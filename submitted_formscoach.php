...
<html>
	<head>
	<meta charset="UTF-8">
	<link rel="stylesheet" type="text/css" href="submitted_forms.css">
	</head>
	<body>
		<div class="wrapper">
...

			{
				...
				echo '<select  onchange="confirmSelection(this)"name="par" id="myListbox" required>';
					echo'<option name= "par"value="" selected="selected">PENDING</option>';
					echo'<option name= "par"value="ACCEPT">ACCEPT</option>';
					echo'<option name= "par"value="REJECT">REJECT</option>';
				echo '</select>';	
...
<script>
                ...
                for (i = 0; i < coll.length; i++) {
                  coll[i].addEventListener("click", function() {
					...
					alert(textContent);
					function sendVariableToPHP(textContent) {
						  var xhr = new XMLHttpRequest();
						  xhr.open("POST", "submitted_formphp.php", true);
						  ...
						  xhr.send("textContent=" + textContent);
						}
					sendVariableToPHP(textContent);
